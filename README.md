...
```bash
# 1. Remediate an existing audio folder (fix bitrates, replace live cuts with studio masters):
audizap "C:\MyMusic\Playlist" --remediate --bitrate 128k --workers 4

# 2. Download from a Spotify Playlist or Album:
audizap "https://open.spotify.com/playlist/6kKAHaM396ytVUggvkM0qp" --bitrate 128k --workers 4

# 3. Download from a YouTube Playlist:
audizap "https://youtube.com/playlist?list=PLfdFekLOHzDo" --bitrate 128k --workers 4

# 4. Download with authenticated session cookies (bypasses bot checks & unlocks 256k AAC):
audizap "https://open.spotify.com/playlist/6kKAHaM396ytVUggvkM0qp" --cookies ./cookies.txt

# 5. Strict Acoustic QC (forces 30s preview cross-correlation on every song):
audizap "My Spotify Library.txt" --strict-qc --bitrate 128k

# 6. Audit & enrich tags on existing files (missing art/lyrics only):

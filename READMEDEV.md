...
This document details the internal architecture, lifecycle flow, module breakdown, and extension points for developers contributing to or adapting this music pipeline.

---

## 🏗️ Architecture & Per-Song Concurrency Model

Rather than executing stage-by-stage across the entire batch (which causes slow serialization and prevents real-time per-song enrichment), the engine delegates complete end-to-end responsibility to individual worker threads:
...
### 2. `pipeline/metadata.py` (`MetadataResolver`)
...
### 3. `pipeline/audio.py` (`AudioResolver`)
...
### 4. `pipeline/transcoder.py` (`Transcoder`)
...
### 6. `pipeline/engine.py` (`PipelineEngine`)
...
In [`pipeline/audio.py`](file:///c:/Users/USER/Desktop/APPS/Stake_29/spotify_downloader/pipeline/audio.py), add your new tier to `download_stream()`:
```python
...
In [`pipeline/tagger.py`](file:///c:/Users/USER/Desktop/APPS/Stake_29/spotify_downloader/pipeline/tagger.py), extend `fetch_lyrics()` with additional providers or custom fallback endpoints:
```python
@classmethod
...
### Customizing Target Audio Codec (e.g. FLAC or Opus)
In [`pipeline/transcoder.py`](file:///c:/Users/USER/Desktop/APPS/Stake_29/spotify_downloader/pipeline/transcoder.py) and [`pipeline/config.py`](file:///c:/Users/USER/Desktop/APPS/Stake_29/spotify_downloader/pipeline/config.py), modify the FFmpeg command line arguments:
```python
# For FLAC lossless output:
cmd = [self.ffmpeg, "-y", "-i", input_path, "-c:a", "flac", temp_output]
```
...
Run the test suite on sample tracks:
```bash
python -c "
...
config = PipelineConfig(output_dir='./test_run', workers=2, bitrate='128k', overwrite_existing=True)
engine = PipelineEngine(config)
queue = ManifestParser.parse_input('Aimee Carty - Painter')
